...
abstract class AbstractDynamicBakedModel : IDynamicBakedModel {
    ...
    val halfVector3f = v(.5f, .5f, .5f)

    /**
     * @return Vector3f(x, y, z)
     */
    protected fun v(x: Float, y: Float, z: Float) = Vector3f(x, y, z)
    ...
    protected fun putVertex(
        ...
        for (j in 0 until elements.size) {
            ...
            when (e.usage) {
                Usage.POSITION -> builder.put(j, vec.x(), vec.y(), vec.z(), 1.0f)
                Usage.COLOR -> builder.put(j, r, g, b, 1.0f)
                Usage.UV -> when (e.index) {
                    0 -> {
                ...
                Usage.NORMAL -> builder.put(j, normal.x(), normal.y(), normal.z())
                else -> builder.put(j)
            }
        }
    }

    protected fun createQuad(
        topLeft: Vector3f,
        bottomLeft: Vector3f,
